 ...
 *
 * 循环一趟需要从数组的零角标依次和当前角标的下一个元素进行比较，然后角标后移，一直循环到最后一个元素的前一位，因为循环的时候每个元素都会和
 ...
 * 冒泡排序的时间复杂度普遍说法为O(n²) ，但其实按照上面逻辑来看更能准确描述的应该是O(n*(n-1))，这个是毫无任何优化的情况下，-1是为了避免数组越界不得不-的啊，并没有
 ...
 * 1. 比较相邻两个元素一直到最后一个元素来进行换位的循环中，第一次循环只需要循环数组长度-1次，理由上面已经说了，而确定第一个元素的时候其实
